    ...
    try:
        with open(plik_instancji, 'r') as f:
            ...
            for i in range(1, n + 1):
                ...
                # Wskaźnik pomocniczy do sortowania początkowego
...
def oblicz_koszt_harmonogramu(harmonogram, zadania_dict):
    ...
    for sekwencja in harmonogram:
        ...
        for id_zad in sekwencja:
            ...
            # Zadanie zaczyna się max(kiedy maszyna wolna, r_j)
            start = max(czas_wolny, z['r'])
            # Zadanie trwa p_j i jest nieprzerwalne
            koniec = start + z['p']
...
def rozwiazanie_startowe_wspt(n, lista_zadan):
    ...
    while pozostale:
        # Wybierz maszynę, która jest najszybciej wolna
        idx_maszyny = min(range(liczba_maszyn), key=lambda i: czasy_zwolnienia[i])
        t_gotowosci = czasy_zwolnienia[idx_maszyny]

        # Znajdź zadania, które już "przyszły" (r <= t)
        dostepne = [z for z in pozostale if z['r'] <= t_gotowosci]

        if dostepne:
            # Z dostępnych wybierz to o najlepszym stosunku czasu do wagi (WSPT)
            wybrane = min(dostepne, key=lambda z: z['wspt'])
        else:
            # Jeśli nic nie jest dostępne, weź to, które pojawi się najszybciej
...
def main():
    ...
    print(f"Czas obliczeń:  {czas_calkowity:.4f} s")  # <--- Tutaj jest wymagany print
